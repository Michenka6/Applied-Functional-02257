...
let rec simulate1 (delta: float) (rxns: Rxns list list) (state: State) : seq<State> =   
    seq {
            //let state = simulateRxns rungeKutta slope delta rxns state 
            yield state 
            yield! simulate1 delta rxns (rxns |> List.fold (fun state rxn -> simulateRxns trapezoidal  slope delta rxn state) state) //(simulateRxns trapezoidal slope delta rxns state)
    }
